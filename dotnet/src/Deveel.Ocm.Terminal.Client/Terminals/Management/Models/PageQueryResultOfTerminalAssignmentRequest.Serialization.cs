...
namespace Deveel.Messaging.Terminals.Management.Models
{
    ...
    {
        internal static PageQueryResultOfTerminalAssignmentRequest DeserializePageQueryResultOfTerminalAssignmentRequest(JsonElement element)
        {
            int totalItems = default;
            Optional<IReadOnlyList<TerminalAssignmentRequest>> items = default;
            ...
            foreach (var property in element.EnumerateObject())
            {
                ...
                if (property.NameEquals("items"))
                {
                    ...
                    List<TerminalAssignmentRequest> array = new List<TerminalAssignmentRequest>();
                    foreach (var item in property.Value.EnumerateArray())
                    {
                        array.Add(TerminalAssignmentRequest.DeserializeTerminalAssignmentRequest(item));
                    }
                    items = array;
                    continue;
                }
            ...
            return new PageQueryResultOfTerminalAssignmentRequest(totalItems, Optional.ToList(items), totalPages, query, self.Value, first.Value, next.Value, previous.Value, last.Value);
        }
    }
}
